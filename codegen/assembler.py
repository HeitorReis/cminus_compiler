...
class Instruction:
    ...
    def disassemble(self, line: str) -> dict:
        ...
        if op_part in branch_ops:
            is_immediate = True if 'i' in op_part else False
            print(f"[DISASSEMBLE] -> Instrução de branch identificada: '{op_part}'")
            details['opcode'] = 'b'
            details['cond'] = op_part[1:] if len(op_part) > 1 else 'do'
            details['type'] = '11'
            details['supp'] = 'i' if is_immediate else 'na'
            details['op2'] = rest_part
            return details
    ...
    def get_unsigned_binary(self, value_str: str, bits: int) -> str:
        """
        Converte uma string numérica para binário sem sinal, com largura fixa em 'bits'.
        Se o valor for negativo ou exceder o intervalo [0, 2**bits-1], retorna um erro.
        """
        try:
            value = int(value_str, 0)
            max_val = (1 << bits) - 1
            if not (0 <= value <= max_val):
                return f"Error: Value '{value}' out of range for {bits}-bit unsigned (0..{max_val})"
            return format(value, f'0{bits}b')
        except (ValueError, TypeError):
            return f"Error: Invalid immediate value '{value_str}'"
        ...
        if d['type'] == '11':
                ...
                try:
                    ...
                    if 'bl' in d['opcode']:
                        ...
                        offset_bin = ('0'*10)+self.get_unsigned_binary(str(offset), 5)+('0'*5)
