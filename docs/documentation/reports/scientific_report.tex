...
\title{Relatório Científico de Desenvolvimento, Correção e Verificação do Sistema Processador ARM-like e Compilador C-minus}
...
\begin{abstract}
Este relatório apresenta a arquitetura, a lógica de implementação, a correção e a verificação de um sistema didático formado pelo processador ARM-like \texttt{Processor(v2026-1)} e pelo compilador C-minus integrado ao seu fluxo de desenvolvimento. O trabalho adota como especificação normativa o documento \texttt{ImportantAdditionalText-ProcessorInformation.txt}, que define uma ISA de 32 bits com largura fixa, campo de condição, bits de suporte para imediato e atualização do CPSR, e organização uniforme dos campos \texttt{Rd}, \texttt{Rh} e \texttt{Operand\_2}. A contribuição principal desta etapa foi alinhar o toolchain ao processador, cobrindo parser, tabela de símbolos, análise semântica, geração de IR, geração de assembly, montagem em código de máquina e simulação. Além da descrição detalhada do desenvolvimento, o relatório documenta os erros encontrados, as correções aplicadas, a metodologia de validação e a interface atual entre compilador, montador, simulador e hardware.
\end{abstract}

\section{Introdução}
O eixo deste projeto é um processador didático ARM-like, descrito em Verilog, que recebe programas por meio de uma memória de instruções e executa uma ISA própria de 32 bits. O compilador C-minus foi incorporado a esse projeto para reduzir a distância entre programas de alto nível e a validação do hardware, permitindo gerar assembly e código de máquina compatíveis com a implementação atual do \texttt{Processor(v2026-1)}.

O compilador C-minus é organizado como um pipeline clássico dividido em frontend, IR e backend. O frontend, implementado em C com Flex e Bison, realiza análise léxica, sintática e semântica. Em seguida, o compilador gera uma representação intermediária textual, usada como entrada para um backend em Python responsável por produzir assembly e, por fim, código de máquina. O objetivo desta correção foi tornar esse pipeline consistente com a ISA ARM-like normativa descrita em \texttt{ImportantAdditionalText-ProcessorInformation.txt} e com o comportamento atualmente auditado no processador. O relatório diferencia o que pertence ao compilador, ao montador, ao simulador em Python e ao RTL para evitar misturar decisões de software com sinais de hardware.

\section{Objetivos e escopo}
Os objetivos técnicos desta etapa foram:
\begin{itemize}[leftmargin=1.5em]
    \item documentar o sistema como uma integração entre processador, compilador, montador e simulador;
...
O escopo cobre o processador atual, o compilador, sua documentação e os pontos de integração entre software e hardware. Portanto, este texto distingue:
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{correções internas do toolchain}, que foram implementadas e validadas; e
    \item \textbf{contratos de integração com o hardware}, que descrevem como o binário gerado se relaciona com a ROM, a RAM, o PC, o CPSR e a lógica de branch/link.
\end{itemize}
...
No caminho atualmente usado pelo compilador, instruções de memória são emitidas com endereço em registrador, no formato conceitual \texttt{load: rd = [ro]} e \texttt{store: [ro] = rh}. Isso acompanha o RTL de \texttt{DataMemory.v}, cuja porta de endereço recebe o valor de \texttt{Ro}.

\subsection{Convenção de software adotada}
Embora a ISA normativa preveja instruções de \emph{branch \& link}, o compilador preserva uma convenção de software explicita para nomes especiais de registradores:
\begin{itemize}[leftmargin=1.5em]
    \item \texttt{r0}: valor de retorno;
    \item \texttt{r1--r3}: três primeiros argumentos;
    \item \texttt{r28}: registrador de link lógico usado no assembly gerado;
    \item \texttt{r29}: \emph{stack pointer};
    \item \texttt{r31}: \emph{frame pointer}.
\end{itemize}
No RTL atual, \texttt{bl} também grava um registrador de link dedicado dentro de \texttt{registerBank.v}. Assim, \texttt{b: r28} significa branch por registrador geral, enquanto o retorno pelo link dedicado é uma classe de branch separada, controlada por \texttt{Funct[22]}.

\subsection{Modelo de memória do toolchain}
O compilador foi unificado para assumir uma pilha lógica de 64 palavras, com \texttt{sp} inicial em 63 e seção de dados posicionada a partir de \texttt{DATA\_SECTION\_BASE = 64} no namespace do back-end. No RTL e no simulador, a RAM de dados possui 64 posições; portanto, a porta de endereço efetiva usa 6 bits, em coerência com \texttt{DataMemory.v}.

\section{Arquitetura do compilador}
Uma descricao modular e mais detalhada das funcoes, classes e matrizes de decisao do projeto esta em \texttt{docs/documentation/compiler\_modules.md}. Este relatorio resume essa documentacao para manter o texto cientifico legivel.
...
\section{Modelos SysML do compilador}
Os modelos SysML foram atualizados no arquivo \texttt{vpp/cminus-compiler-expanded.vpp} para mostrar o compilador como a ferramenta que faz a ponte entre o programa em C-minus e o código binário de 32 bits aceito pela arquitetura ARM simplificada. O processador aparece apenas como arquitetura-alvo; o foco dos diagramas fica no compilador.

O \textbf{Diagrama de Atividades - Fluxo de Compilação C- para Código Binário ARM Simplificado} mostra o caminho completo da compilação. Ele começa em \texttt{Início da compilação}, recebe o código C-, faz análise léxica, sintática e semântica, consulta a tabela de símbolos, gera código intermediário, mapeia variáveis e temporários para registradores, gera assembly, resolve rótulos, codifica as instruções em binário de 32 bits, valida o resultado e gera o arquivo final \texttt{.txt}. Esse diagrama usa colunas de responsabilidade, ou \emph{swimlanes}, para separar os módulos: entrada do usuário, analisadores, tabela de símbolos, gerador de IR, alocador de registradores, gerador de assembly, resolvedor de rótulos, codificador binário e validador/gerador de arquivo.

O mesmo diagrama também mostra os fluxos alternativos de erro. Se os tokens forem inválidos, é gerado erro léxico. Se a sintaxe estiver errada, é gerado erro sintático. Se a semântica falhar, é gerado erro semântico. Se o binário final tiver tamanho ou campos inválidos, é gerado erro de codificação. Todos esses fluxos passam por \texttt{Gerar relatório de erro} e chegam ao \texttt{Fim da compilação}.

O \textbf{Diagrama de Blocos - Arquitetura Interna do Compilador} representa a estrutura interna do compilador. O bloco principal é \texttt{Compilador C- para ARM Simplificado}, contendo os blocos de gerenciamento, análise léxica, análise sintática, análise semântica, tabela de símbolos, geração de código intermediário, alocação de registradores, geração de assembly, resolução de rótulos, codificação binária, validação e geração do arquivo de saída. O gerador de assembly mostra as instruções consideradas no modelo: \texttt{mov}, \texttt{add}, \texttt{sub}, \texttt{subs}, \texttt{mul}, \texttt{div}, \texttt{and}, \texttt{or}, \texttt{xor}, \texttt{not}, \texttt{load}, \texttt{store}, \texttt{b}, \texttt{bl}, \texttt{bgt}, \texttt{blt}, \texttt{beq} e \texttt{bneq}. O codificador binário foi detalhado com os sub-blocos \texttt{CondEncoder}, \texttt{TypeEncoder}, \texttt{SuppEncoder}, \texttt{FunctEncoder}, \texttt{RegisterEncoder} e \texttt{Operand2Encoder}.

O \textbf{Diagrama de Blocos — Hierarquia dos Módulos do Compilador} mostra a organização do código usando os nomes reais do repositório. Ele separa o controle principal em \texttt{main.c}, a análise léxica e sintática em \texttt{parser/lexer.l} e \texttt{parser/parser.y}, as estruturas e análises do frontend em \texttt{src/}, e o backend em \texttt{codegen/}. Dentro desses blocos aparecem estruturas e funções reais, como \texttt{AstNode}, \texttt{SymbolTable}, \texttt{semanticAnalyze}, \texttt{generate\_ir}, \texttt{RegisterAllocator}, \texttt{FunctionContext} e \texttt{FullCode}. As setas desse diagrama não representam passo a passo de execução; elas indicam dependência entre arquivos, por exemplo o parser depender dos tokens do lexer, a análise semântica depender da AST e da tabela de símbolos, e o backend Python depender do arquivo \texttt{generated\_IR.txt}.

O \textbf{Diagrama Interno - Codificador Binário} separa essa última etapa para deixar o formato da instrução mais fácil de ler. Ele mostra que o codificador recebe \texttt{assemblyResolvido}, passa pelos encoders de condição, tipo, suporte, função, registradores e \texttt{Operand2}, e entrega \texttt{instrucoesBinarias} para o validador. Essa visão evita misturar a arquitetura interna do processador com o trabalho do compilador; o hardware aparece apenas como formato-alvo aceito pelo binário.

Nos modelos que envolvem a codificação binária, a instrução final segue o formato normativo:
\begin{center}
\texttt{Cond[31:28] | Type[27:26] | Supp[25:24] | Funct[23:20] | Rd[19:15] | Rh[14:10] | Operand\_2[9:0]}
\end{center}

O \textbf{Diagrama de Rastreabilidade - C- para IR, Assembly e Binário} complementa os modelos com um exemplo pequeno de \texttt{input()}, comparação e \texttt{output()}. Ele mostra o encadeamento \texttt{Código C- -> Código intermediário -> Assembly -> Código binário}. No binário conceitual, o caractere \texttt{x} indica campos de imediato ou deslocamento que dependem da resolução de rótulos e da posição final da instrução.

Assim, os diagramas deixam claro que o compilador traduz C-minus para IR, depois para assembly da arquitetura-alvo e finalmente para uma linha binária de 32 bits por instrução, pronta para ser usada como \texttt{single\_port\_rom\_init.txt} ou arquivo \texttt{.txt} equivalente de inicialização da memória de instruções. Os modelos antigos de fases isoladas e cópias do frontend/backend foram removidos do arquivo \texttt{.vpp} para não deixar versões duplicadas ou contraditórias.
...
Modelo de memória inconsistente & constantes divergentes entre backend e assembler & constantes compartilhadas: pilha de 64 palavras, \texttt{sp}=63 e base de dados 64 & endereçamento interno unificado \\
\bottomrule
\end{longtable}
...
As seguintes limitações e cuidados permanecem relevantes:
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Link lógico e link dedicado}: o assembly usa o nome \texttt{lr} para \texttt{r28}, mas o RTL também possui um registrador de link dedicado para instruções \texttt{bl} e retorno por link.
    \item \textbf{Modelo de memória}: o back-end separa pilha e dados por convenção textual, enquanto a RAM efetiva do processador tem 64 endereços; por isso a carga na ROM/RAM deve respeitar o mapeamento de 6 bits do hardware.
    ...
    \item \textbf{Logs verbosos}: o front-end e o back-end ainda mantêm mensagens de depuração extensas, úteis para auditoria, mas ruidosas para uma ferramenta final.
\end{itemize}

\section{Trabalhos futuros}
Os próximos passos tecnicamente recomendados são:
\begin{itemize}[leftmargin=1.5em]
    \item consolidar a ABI para decidir quando usar \texttt{r28} como link lógico e quando usar o retorno por link dedicado do RTL;
    \item automatizar a atualização de \texttt{Processor(v2026-1)/modules/single\_port\_rom\_init.txt} a partir do binário gerado;
...
Modelo de memória do compilador & consistente entre fases & implementado & pilha de 64 palavras e RAM efetiva de 64 endereços \\
Compatibilidade com \texttt{Processor(v2026-1)} & desejável & parcial & branch/link e tamanho de memória documentados; carga automatica da ROM ainda e trabalho futuro \\
\bottomrule
\end{longtable}
...
\begin{itemize}[leftmargin=1.5em]
    \item \texttt{docs/generated/frontend/syntax\_tree/ast.txt};
    \item \texttt{docs/generated/frontend/symbol\_table/symbol\_table.txt};
    \item \texttt{docs/generated/intermediate/semantic/ir/generated\_IR.txt};
    \item \texttt{docs/generated/intermediate/codegen/assembly/generated\_assembly.txt};
    \item \texttt{docs/generated/final/assembler/machine\_code/generated\_machine\_code.txt};
    \item \texttt{docs/generated/batch/final\_machine\_code/}.
\end{itemize}
